document.addEventListener("DOMContentLoaded", (_) => {
  ...
  const oldPassRegex =
    /(?:existing|old|curr|former)(?=.*pass)|(?<=pass.*)(?:existing|old|curr|former)/;
  let activeIcons: { icon: HTMLElement; input: HTMLElement }[] = [];
  const activeOverlays: {
    overlay: HTMLElement;
    icon: { icon: HTMLElement; input: HTMLElement };
  }[] = [];
  ...
  function startTrackingPageChanges() {
    ...
    document.addEventListener("click", (e: MouseEvent) => {
      ...
      if (activeOverlays.length > 0 && !(e.target as HTMLElement).classList.contains("bitwarden"))
        closeAllOverlays();
    });
      ...
      setTimeout(() => {
        const haveInputsChanged = mutations.some((mutation) => {
          return [...mutation.addedNodes, ...mutation.removedNodes].some((node) => {
            ...
            return (
              changedNode.tagName &&
              (changedNode.tagName === "INPUT" || changedNode.getElementsByTagName("INPUT"))
            );
          });
        });
  ...
  function hasAttribute(el: HTMLElement, attr: string) {
    return Array.from(el.attributes).some(
      (att) => att.name.toLowerCase().includes(attr) || att.value.toLowerCase().includes(attr)
    );
  }
  ...
  function createIcons() {
    ...
    inputs.filter(isValidField).forEach((input) => {
      if (input.form) {
        ...
        const usernameFields = formInputs.filter(
          (field) => isUsernameField(field) && isValidField(field)
        );
        const passwordFields = formInputs.filter(
          (field) => field.type.toLowerCase() === "password" && isValidField(field)
        );
        ...
        const isSignupOrCpwForm =
          passwordFields.length > 1 ||
          usernameFields.length > 1 ||
          newPwFields.length > 0 ||
          hasAttribute(input, "signup") ||
          hasAttribute(input.form, "signup");

        if (!isSignupOrCpwForm) {
          placeIcon(input, passwordFields.includes(input) ? "password" : "username");
        } else {
          const oldPwFields = passwordFields.filter(
            (field) =>
              field.autocomplete.includes("current") ||
              field.name.match(oldPassRegex) ||
              field.id.match(oldPassRegex)
          );
  ...
  function calculateIconPos(
    ...
    const parentPosition =
      input.offsetParent && input.offsetParent.tagName !== "BODY"
        ? input.offsetParent.getBoundingClientRect()
        : { left: 0, top: 0 };
    if (!inputPosition) return;

    return {
      left:
        inputPosition.left + inputPosition.width - inputWidth - marginRight - parentPosition.left,
  ...
  function checkPositions() {
    activeIcons.forEach((icon) => {
      ...
      if (document.body.contains(icon.input)) {
        ...
        if (position) {
          if (position.left !== parseInt(iconElement.style.left, 10))
            iconElement.style.left = position.left + "px";
          if (position.top !== parseInt(iconElement.style.top, 10))
    ...
    activeOverlays.forEach((overlay) => {
      ...
      if (document.body.contains(overlay.icon.icon)) {
        ...
        if (position) {
          if (position.left !== parseInt(overlayElement.style.left, 10))
            overlayElement.style.left = position.left + "px";
          if (position.top !== parseInt(overlayElement.style.top, 10))
            overlayElement.style.top = position.top + "px";
          if (position.width !== parseInt(overlayElement.style.width, 10))
            overlayElement.style.width = position.width + "px";
          if (position.height !== parseInt(overlayElement.style.height, 10))
  ...
  function placeIcon(input: HTMLElement, type = "username") {
    ...
    switch (type) {
      case "newPassword":
        ...
        break;
      case "username":
        if (cipherData.usernameCiphers > 0) imagePath = "shield-blue";
        break;
      case "password":
        if (cipherData.passwordCiphers > 0) imagePath = "shield-blue";
        break;
      default:
        if (cipherData.ciphers > 0) imagePath = "shield-blue";
        break;
    }
    ...
    const styleOverrides: { [key: string]: string } = {
      ...
      background:
        "url(" +
        chrome.runtime.getURL(`images/${imagePath}.svg`) +
  ...
  function calcualteOverlayPos(input: HTMLElement, marginLeft = 0, marginTop = 5) {
    ...
    return {
      left: window.pageXOffset + inputPosition.left + marginLeft,
